...
export class MediaWatching extends HTMLElement {
  ...
  _completedIds = new Set(); // IDs of episodes that finished; prevents stop/play from re-adding them
  ...
  _handlePlayVideoEvent = async (eventData) => {
    ...
    // Ignore progress events for episodes that already completed and
    // were removed — the stop/close chain can still fire stale events.
    if (this._completedIds.has(payload._id)) return;
  ...
  _handleStopVideoEvent = async (eventData) => {
    ...
    // Don't re-save / re-append an entry that was already completed
    // and removed — the player close fires a stop event AFTER the
    // completion handler already cleaned up the card.
    if (this._completedIds.has(payload._id)) return;
  ...
  async _processRemovalPayload(payload) {
    ...
    // Mark this ID as completed so that subsequent stop/play events
    // (fired by the player close chain) won't re-save or re-append the card.
    if (payload.completed) {
      this._completedIds.add(payload._id);
    }

    const now = Date.now();
    const lastTimestamp = this._recentRemovalTimestamps.get(payload._id) || 0;
    // Allow completion events through even during the dedup window —
    // otherwise the REMOVE event (without completed) runs first and
    // blocks the completion event that carries the flag needed by
    // _addNextEpisodeToWatching.
    if (now - lastTimestamp < 2500 && !payload.completed) {
      return;
    }
  ...
  async _loadInitialWatchingTitles() {
    try {
      ...
      // Load all cards in parallel instead of sequentially.
      // Each appendTitle creates a card and fetches media info concurrently,
      // so N cards load in ~1 round-trip time instead of N sequential ones.
      const BATCH_SIZE = 6;
      for (let i = 0; i < titles.length; i += BATCH_SIZE) {
        const batch = titles.slice(i, i + BATCH_SIZE);
        await Promise.allSettled(
          batch.map((title) => this.appendTitle(title))
        );
      }
    } catch (err) {
...
export class MediaWatchingCard extends HTMLElement {
  ...
  async setTitle(titleData, callback, errorCallback) {
    ...
    try {
      ...
      // Populate basic info synchronously from the media object (text, meta).
      // Thumbnail search and other I/O-heavy work runs in background.
      this._populateBasicInfoSync(entryType);
      this._updateProgress();

      // Resolve the callback early so the card appears immediately
      // while thumbnails, series info, and preview load in background.
      if (callback) callback();

      // Fire background enrichment tasks concurrently (non-blocking)
      this._populateBasicInfoAsync(entryType);
  ...
  // Synchronous part of card population — sets title text, meta, and
  // thumbnail from already-available data (no network calls).
  _populateBasicInfoSync(entryType) {
    ...
    // thumbnail — use what's already available, no I/O
    ...
    if (this.thumbnailImg) {
      if (thumb) {
        ...
        // placeholder while async thumbnail search runs
        this.thumbnailImg.src =
          "data:image/svg+xml;base64," +
          btoa(
            `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="225"><rect width="100%" height="100%" fill="#222"/><text x="50%" y="50%" fill="#888" font-size="16" font-family="sans-serif" text-anchor="middle">Loading...</text></svg>`
          );
        this._needsThumbnailSearch = true;
      }
    }
  ...
  // Async part — searches for local thumbnail if none was found synchronously.
  // Runs in background after the card is already visible.
  async _populateBasicInfoAsync(entryType) {
    if (!this._needsThumbnailSearch) return;
    this._needsThumbnailSearch = false;
    try {
      const thumb = await this._findLocalThumbnail(entryType);
      if (thumb && this.thumbnailImg) {
        this.thumbnailImg.src = thumb;
      } else if (this.thumbnailImg && this.thumbnailImg.src.includes("Loading...")) {
        this.thumbnailImg.src =
          "data:image/svg+xml;base64," +
          btoa(
      ...
      // ignore — card already shows placeholder
    }
  }
  ...
  async _getFilePaths(entryType = "video") {
    if (this._cachedFilePaths != null) return this._cachedFilePaths;
    const id = this.titleData?.titleId || this.titleData?._id;
    if (!id) return [];
    const indexPath = entryType === "title" ? INDEX_TITLES : INDEX_VIDEOS;
    this._cachedFilePaths = await getTitleFiles(id, indexPath).catch(() => []);
  ...
  async _updateEpisodeSeriesInfo() {
    ...
    // Check type: must be TVEpisode, or have a Serie identifier.
    const serieIdEarly = this._extractSeriesIdentifier();
    if (typeName !== "TVEpisode" && !serieIdEarly) {
      return;
    }

    const fallbackName = this._guessSeriesDisplayName();
    const serieId = serieIdEarly || this._extractSeriesIdentifier();
    if (!serieId) {
      if (fallbackName) {
