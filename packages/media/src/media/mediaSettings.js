...
import {
  ...
  getTitleServiceTmdbApiKey,
  setTitleServiceTmdbApiKey,
} from "@globular/sdk";
...
class MediaSettings extends HTMLElement {
        ...
        <div class="sections">
          ...
          <div class="section">
            ...
            <div class="section-body">
              <div class="row">
                <div class="field-group">
                  <input id="tmdb-api-key" type="text" placeholder="Enter your TMDb API key" style="min-width:260px;" />
                  <button id="btn-save-tmdb-key" class="btn-small">Save</button>
                </div>
              </div>
              <div class="row" style="font-size:0.75rem; opacity:0.8;">
                <span>Required for title enrichment (genres, cast, description). Get one at themoviedb.org.</span>
              </div>
            </div>
          </div>
  ...
  bindEvents() {
    ...
    // TMDb API Key
    const btnSaveTmdbKey = $("btn-save-tmdb-key");
    const tmdbKeyInput = $("tmdb-api-key");
    btnSaveTmdbKey?.addEventListener("click", async () => {
      const key = (tmdbKeyInput?.value || "").trim();
      if (!key) {
        displayError("Please enter a TMDb API key.", 3000);
        return;
      }
      try {
        await setTitleServiceTmdbApiKey(key);
        displayMessage("TMDb API key saved. Restart the title service for it to take effect.", 4000);
      } catch (err) {
        displayError(err?.message || err, 4000);
      }
    });
  ...
  async refreshAll() {
    ...
    await this.loadTmdbApiKey();
    await this.refreshWorkerStatus();
    await this.loadLogs();
    await this.loadErrors();
  }

  async loadTmdbApiKey() {
    const input = this.shadowRoot.getElementById("tmdb-api-key");
    if (!input) return;
    try {
      const key = await getTitleServiceTmdbApiKey();
