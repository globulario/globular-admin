...
import { getTitleFiles, getTitleInfo, buildFileUrl, readDir, removeWatchingTitle, saveWatchingTitle } from "@globular/sdk";
...
export async function playTitleListener(player, currentEpisode, indexPath = INDEX_TITLES) {
  ...
  // Pre-fetch the episode list + next episode file paths while the user is
  // still watching, so the "play next" prompt appears instantly when the
  // video ends instead of waiting 20-30 s for a network round-trip.
  let prefetchedNext = null;
  const serieId = typeof currentEpisode.getSerie === "function" ? currentEpisode.getSerie() : null;
  if (serieId) {
    searchEpisodes(serieId, indexPath)
      .then(async (episodes) => {
        const idx = episodes.findIndex((e) => e.getId() === currentEpisode.getId());
        const next = episodes[idx + 1];
        if (!next) return;
        // Also pre-fetch the video file path for the next episode.
        const files = await getTitleFilePaths(indexPath, next).catch(() => []);
        prefetchedNext = { episode: next, filePaths: files };
      })
      .catch(() => {}); // best-effort; onended will retry if this fails
  }

  player.media.onended = async () => {
    _exitFullscreen();

    // Explicitly remove the finished episode from watching history.
    // The close/stop chain may not detect completion because this handler
    // overwrites the player's built-in onended (which sets _playbackCompleted).
    const episodeId = currentEpisode.getId();
    removeWatchingTitle(episodeId).catch(err =>
      console.warn("playTitleListener: failed to remove watching entry", err)
    );
    localStorage.removeItem(episodeId);

    // Also set _playbackCompleted so the stop() path doesn't re-save it.
    const gp = document.getElementsByTagName("globular-video-player")[0];
    if (gp) {
      gp._playbackCompleted = true;
      gp._watchingRemovedAfterCompletion = true;
      if (typeof gp.close === "function") gp.close();
    }

    if (!serieId) return;

    // Use pre-fetched data if available, otherwise fall back to a live query.
    try {
      let nextEpisode, filePaths;
      if (prefetchedNext) {
        nextEpisode = prefetchedNext.episode;
        filePaths = prefetchedNext.filePaths;
      } else {
        const episodes = await searchEpisodes(serieId, indexPath);
      ...
      // Add the next episode to watching immediately so it appears in
      // the Continue Watching list even if the user dismisses the dialog.
      const nextId = nextEpisode.getId();
      if (nextId) {
        const nextEntry = {
          titleId: nextId,
          _id: nextId,
        ...
        const dur = typeof nextEpisode.getDuration === "function" ? nextEpisode.getDuration() : undefined;
        if (typeof dur === "number" && Number.isFinite(dur) && dur > 0) {
          nextEntry.duration = dur;
          nextEntry.duration_ms = Math.round(dur * 1000);
        }
        saveWatchingTitle(nextEntry).catch(err =>
          console.warn("playTitleListener: failed to persist next episode to watching", err)
        );
      }

      await _promptPlayNextEpisode(nextEpisode, indexPath, filePaths);
    } catch (err) {
...
async function _promptPlayNextEpisode(nextEpisode, indexPath = INDEX_TITLES, prefetchedFilePaths) {
  ...
  return new Promise((resolve, reject) => {
    ...
    okBtn.addEventListener("click", async () => {
      if (toast.hideToast) toast.hideToast();
      try {
        const filePaths = (prefetchedFilePaths && prefetchedFilePaths.length > 0)
          ? prefetchedFilePaths
          : await getTitleFilePaths(indexPath, nextEpisode);
