...
async function _searchEpisodesForSerie(serieId, indexPath) {
  // Use dedicated backend RPC first — authoritative, sorted.
  try {
...
export async function playTitleListener(player, currentEpisode, indexPath) {
  ...
  if (serieId) {
    _searchEpisodesForSerie(serieId, indexPath)
      .then(async (episodes) => {
  ...
  player.media.onended = async () => {
    ...
    if (globalPlayer) {
      globalPlayer._playbackCompleted = true;
      globalPlayer._watchingRemovedAfterCompletion = true;
      if (typeof globalPlayer.close === "function") globalPlayer.close();
    }
    ...
    try {
      ...
      if (prefetchedNext) {
        ...
        const episodes = await _searchEpisodesForSerie(serieId, indexPath);
 ...
 * ------------------------------------------------------------------------------------------------*/
async function _promptPlayNextEpisode(nextEpisode, indexPath, prefetchedFilePaths) {
  return new Promise((resolve, reject) => {
