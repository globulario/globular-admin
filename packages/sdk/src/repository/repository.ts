...
export async function fetchInstalledPackages(
  ...
  if (!resp.ok) {
    if (resp.status === 401) throw new Error("Your session token is no longer valid. Please log out, log back in, and try again.")
    throw new Error(`admin/packages: HTTP ${resp.status}`)
  }
  const data = await resp.json()
  const pkgs: InstalledPackage[] = data.packages ?? data ?? []
  // Backend returns publisherId (protojson); normalize to publisher for UI.
  for (const p of pkgs) {
    if (!p.publisher && p.publisherId) {
