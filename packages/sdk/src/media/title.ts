...
export async function getTitleInfo(
  ...
  if (title) {
    titlesCache.set(id, title);
    prefetchSeriesTitles([title], indexPath);
  }
  return title;
}
...
/* =====================================================================================
 * Series episodes lookup (backend-authoritative)
 * ===================================================================================== */

/**
 * Fetch all episodes for a given series, sorted by season then episode.
 * Uses the dedicated GetSeriesEpisodes RPC — no search-based hacks.
 * Optionally filter by season (0 = all seasons).
 */
export async function getSeriesEpisodes(
  ...
  rq.setIndexpath(indexPath);
  if (season > 0) rq.setSeason(season);
  ...
  const rsp = await unary(
    ...
    undefined,
    md
  ) as titlepb.GetSeriesEpisodesResponse;

  const episodes = rsp?.getEpisodesList?.() ?? [];

  // Cache each episode by id.
  episodes.forEach((ep) => titlesCache.set(ep.getId(), ep));

  return episodes;
}

/* =====================================================================================
 * Auto-fetch parent series titles
 * ===================================================================================== */

// Track in-flight series prefetches to avoid duplicate requests.
const seriesPrefetchPending = new Set<string>();

/**
 * For any TVEpisode title that has a Serie (parent series IMDb ID),
 * pre-fetch the series title into cache if not already present.
 * Runs in the background — never blocks the caller.
 */
function prefetchSeriesTitles(titles: titlepb.Title[], indexPath: string) {
  for (const t of titles) {
    const type = typeof t.getType === "function" ? t.getType() : "";
    const serieId = typeof t.getSerie === "function" ? t.getSerie() : "";
    if (!serieId || type !== "TVEpisode") continue;
    if (titlesCache.has(serieId)) continue;
    if (seriesPrefetchPending.has(serieId)) continue;

    seriesPrefetchPending.add(serieId);
    getTitleInfo(serieId, indexPath)
      .catch(() => {})
      .finally(() => seriesPrefetchPending.delete(serieId));
  }
}
...
/* =====================================================================================
 * Title service config (TMDb API key)
 * ===================================================================================== */

/** Fetch the TmdbApiKey from the title service config. */
export async function getTitleServiceTmdbApiKey(): Promise<string> {
  try {
    const base = (getBaseUrl() ?? "").replace(/\/$/, "");
    if (!base) return "";
    const res = await fetch(`${base}/config/title.TitleService`, {
      credentials: "include",
    });
    if (!res.ok) return "";
...
/** Save the TmdbApiKey into the title service config (patch). */
export async function setTitleServiceTmdbApiKey(apiKey: string): Promise<void> {
  const base = (getBaseUrl() ?? "").replace(/\/$/, "");
  if (!base) throw new Error("No base URL configured.");

  // First, fetch the current config to get the service Id.
  const cfgRes = await fetch(`${base}/config/title.TitleService`, {
    credentials: "include",
  });
  if (!cfgRes.ok) throw new Error("Failed to read title service config.");
  const cfg = await cfgRes.json();
  const id = cfg.Id || cfg.ID || cfg.id;
  if (!id) throw new Error("Title service config has no Id field.");

  const headers: Record<string, string> = { "Content-Type": "application/json" };
  const token = sessionStorage.getItem("__globular_token__");
  if (token) headers["token"] = token;

  const res = await fetch(`${base}/api/save-service-config`, {
    method: "POST",
    credentials: "include",
    headers,
    body: JSON.stringify({ Id: id, TmdbApiKey: apiKey }),
  });
