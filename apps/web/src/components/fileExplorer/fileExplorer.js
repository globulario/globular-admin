...
// REMOVE: import { FileController } from "../../backend/file"
...
// ✅ Use your existing unified backend wrapper (src/backend/files.ts)
import {
    addPublicDir,
    createDir,
    createLink,
...
/** --- Small adapters so the UI can keep calling proto-like getters --- **/

/** Adapt a FileVM (from backend/files) to an object that looks like a FileInfo (proto-like). */
function adaptFileVM(vm) {
    const f = vm || {};
    // create a stable object with getters
    const obj = {
        // keep raw for views that poke into fields
        __vm: vm,
        getPath: () => f.path || "",
        getName: () => f.name || (f.path ? f.path.substring(f.path.lastIndexOf("/") + 1) : ""),
        getIsDir: () => !!f.isDir,
        getMime: () => f.mime || "",
        getSize: () => (typeof f.size === "number" ? f.size : 0),
        getThumbnailsList: () => Array.isArray(f.thumbnails) ? f.thumbnails : [],
        // allow direct access used by your views
        titles: f.titles,
        videos: f.videos,
        audios: f.audios
    };
    // also mirror common fields directly (a few views directly access f.mime, f.path)
    obj["path"] = f.path;
    obj["name"] = obj.getName();
    obj["isDir"] = obj.getIsDir();
    obj["mime"] = obj.getMime();
    obj["size"] = obj.getSize();
    obj["thumbnails"] = obj.getThumbnailsList();
    return obj;
}

/** Adapt a DirVM to something with getPath() and getFilesList() */
function adaptDirVM(dirVM) {
    ...
    const obj = {
        ...
        // also mirror fields some components access directly
        path: d.path || "",
        files: filesList
    };
    return obj;
...
/** Helper to extract a path from either proto-like or VM-like file/dir objects */
...
export class FileExplorer extends HTMLElement {
    ...
    _initializeComponents() {
        ...
        this._diskSpaceManager.account = AccountController.account;
    ...
    async _loadInitialData() {
        ...
        try {
            const userDir = `/users/${AccountController.account.getId()}@${AccountController.account.getDomain()}`;
            await readAndSetRoot(userDir);

            const applicationDir = `/applications/${window.location.pathname.split('/')[1]}`;
            if (applicationDir !== userDir) {
    ...
    _handleSharePanelClick(evt) {
        ...
        if (this._sharePanel === null) {
            this._sharePanel = new SharePanel(AccountController.account);
    ...
    static _validateDirAccess(dir) {
        const p = extractPath(dir);
        if (!p) return false;
        // Original logic effectively always returned true
        return true;
    }

    setDir(dir, callback) {
        if (!FileExplorer._validateDirAccess(dir)) {
            displayMessage("Access to this directory is not permitted.", 3000);
            return;
        }
    ...
    async playAudio(file) {
        ...
        try {
            const audios = await TitleController.getFileAudiosInfo(file);
