...
import { createOrUpdateAudio } from "../../backend/media/title";
...
export class AudioInfoEditor extends HTMLElement {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        #container {
          ...
          margin:15px 0; padding:15px; box-sizing:border-box;
        }
        .image-column { display:flex; flex-direction:column; align-items:center; flex-shrink:0; }
        .info-column { display:flex; flex-direction:column; flex:1 1 300px; min-width:300px; }
        .info-table { display:table; width:100%; border-collapse:collapse; margin-top:10px; }
        .info-row { display:table-row; border-bottom:1px solid var(--palette-divider, #ddd); }
        .info-row:last-child { border-bottom:none; }
        .label {
          display:table-cell; font-weight:600; padding:8px 10px 8px 0; min-width:120px; vertical-align:middle;
        }
        .value-display, .input-field {
          display:table-cell; width:100%; padding:8px 0; vertical-align:middle;
        }
        .button-cell { display:table-cell; vertical-align:middle; width:48px; }
        .action-div {
        ...
        <div class="info-column">
          <div class="info-table">
            <div class="info-row" style="border-bottom:1px solid var(--palette-divider, #ddd)">
              <div class="label">Audio Information</div>
  ...
  _row(label, key) {
    return `
      <div class="info-row">
        ...
        <div class="input-field"><paper-input id="${key}-input" no-label-float></paper-input></div>
        <div class="button-cell"><paper-icon-button id="edit-${key}-btn" icon="image:edit"></paper-icon-button></div>
      </div>
    `;
  }
  _rowNum(label, key) {
    return `
      <div class="info-row">
        <div class="label">${label}:</div>
        <div class="value-display" id="${key}-div"></div>
        <div class="input-field"><paper-input id="${key}-input" type="number" no-label-float></paper-input></div>
        <div class="button-cell"><paper-icon-button id="edit-${key}-btn" icon="image:edit"></paper-icon-button></div>
      </div>
    `;
  }
  _rowArea(label, key) {
    return `
      <div class="info-row">
        <div class="label" style="vertical-align:top">${label}:</div>
        <div class="value-display" id="${key}-div" style="padding-bottom:10px"></div>
        <div class="input-field"><iron-autogrow-textarea id="${key}-input" no-label-float></iron-autogrow-textarea></div>
        <div class="button-cell" style="vertical-align:top"><paper-icon-button id="edit-${key}-btn" icon="image:edit"></paper-icon-button></div>
      </div>
    `;
  }
  ...
  _wireEditable(displayEl, inputEl, editBtn, setter, type = "text") {
    ...
    const showInput = () => {
      ...
      inputEl.style.display = "table-cell";
      setTimeout(() => {
        const focusEl = type === "textarea" ? inputEl.textarea : inputEl.inputElement?._inputElement;
        if (focusEl) {
          focusEl.focus();
          if (type !== "textarea") focusEl.select?.();
        }
      }, 0);
    };
    const commit = () => {
  ...
  async _handleSave() {
    ...
    try {
      await createOrUpdateAudio(this._audio);
