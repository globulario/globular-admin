...
//
// globular: protects ui.destructive_action_requires_explicit_confirmation (deleteOrganization)
...
export class OrganizationsManager extends HTMLElement {
  ...
  connectedCallback() {
    this.shadow.innerHTML = `
      <style>
        ...
        .page { color: var(--on-surface-color); }
        .header { display:flex; align-items:center; gap:.5rem; margin-bottom:.75rem; }
        h2 { margin:0; font-size:1rem; font-weight:700; text-transform: uppercase; letter-spacing: .04em; color: var(--secondary-text-color); }
        .spacer { flex:1; }
        .card { background: var(--md-surface-container-low); color: var(--on-surface-color); padding: 1rem 1.25rem;
          border-radius: var(--md-shape-md); border: 1px solid var(--border-subtle-color); box-shadow: var(--md-elevation-1); }
  ...
  private async refresh() {
    try {
      ...
      console.error(e);
      displayError(e?.message || "Failed to load organizations.");
      this.table.setData([]);
    }
  }
