...
class PageRepoNamespaces extends HTMLElement {
  ...
  private _pushData() {
    this._set('loading', this._loading ? '<div style="color:var(--secondary-text-color);font-size:.85rem;padding:16px">Loading namespaces...</div>' : '')
    this._set('error', this._error ? `<div style="color:var(--error-color);padding:8px">${esc(this._error)}</div>` : '')
    ...
    this.querySelector('#btnRefresh')?.addEventListener('click', () => { this._loading = true; this._pushData(); this._load() })
    this.querySelector('#btnClaim')?.addEventListener('click', () => { this._showClaimForm = true; this._pushData() })
    ...
    if (search) {
      search.addEventListener('input', () => {
        ...
        this._pushTable()
        ;(this.querySelector('#nsSearch') as HTMLInputElement)?.focus()
      })
    }
  ...
  private _pushTable() {
    ...
    if (list.length === 0) {
      this._set('table', '<div class="empty-state">No namespaces found. Publish a package or claim a namespace.</div>')
      return
    }
    ...
    this.querySelectorAll('[data-claim-quick]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { await claimNamespace(ns); await this._load() }
        catch (e: any) { this._error = e?.message || 'Claim failed'; this._pushData() }
      })
    })
    ...
    this.querySelectorAll('[data-revoke-ns]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (!ok) return
        try { await revokeNamespaceAccess(ns, user); await this._load() }
        catch (e: any) { this._error = e?.message || 'Revoke failed'; this._pushData() }
      })
    })
  ...
  private renderClaimForm() {
    ...
    el.querySelector('#claimCancel')?.addEventListener('click', () => { this._showClaimForm = false; this._pushData() })
    el.querySelector('#claimSubmit')?.addEventListener('click', async () => {
      ...
      if (!name) return
      try { await claimNamespace(name, org || undefined); this._showClaimForm = false; await this._load() }
      catch (e: any) { this._error = e?.message || 'Claim failed'; this._pushData() }
    })
  }
  ...
  private renderGrantForm(nsId: string) {
    ...
    el.querySelector('#grantCancel')?.addEventListener('click', () => { this._showGrantForm = null; this._pushTable() })
    el.querySelector('#grantSubmit')?.addEventListener('click', async () => {
      ...
      if (!user) return
      try { await grantNamespaceAccess(nsId, user, role); this._showGrantForm = null; await this._load() }
      catch (e: any) { this._error = e?.message || 'Grant failed'; this._pushData() }
    })
  }
}
